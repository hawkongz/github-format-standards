...
> **TOC 锚点规则：**
> 1. GitHub 会把标题里的 emoji 删掉，emoji 后面的空格变成 `-`。`## 🔍 功能特性` → 锚点是 `#-功能特性`
> 2. **禁止用变体选择器 emoji（如 ⌨️）**——基字符被删后 U+FE0F 残留为不可见字符，锚点会变成 `#️-xxx` 导致链接失效
> 3. 只使用单码点 emoji（📋 ⚡ 🔧 🚀 等）
> 4. TOC 条目文本不加 emoji，仅标题加 emoji
> 5. 生成 TOC 后必须逐个验证锚点匹配
